  ...
  try {
    ...
    const tasks = new Listr([
      ...
      },
      {
        ...
        async task(ctx) {
          ...
          try {
            ...
            return spawn(`git init && git remote add origin ${ctx.repoSSLUrl}`, [], { cwd: pathToCreate, shell: true })
      ...
      },
      {
        ...
        skip(ctx) {
          if (!ctx.withLuctstCli) return true;
          return false;
        },
        task(ctx, task) {
          const pathToCreate = ctx.editPath ? ctx.editPath : ctx.path;

          const subprocess = spawn('npx luctst-cli', ['start'], { cwd: pathToCreate, shell: true });
